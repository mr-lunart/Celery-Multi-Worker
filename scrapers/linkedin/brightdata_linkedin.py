...
class LinkedinScrapper:
    def __init__(self, bucket_name:str, api_key:str, param_input:dict) -> None:
        ...
        self.platform_channel_name = "linkedin"
    ...
    def start(self):
        ...
        if isinstance(result, dict):
            ...
            return
        self.save_upload_s3(s3_key="linkedin/test_scrapper/")

    def sync_linkedin_post_by_url_company(self, input_data:list[dict]):
        self.request_input['input'] = input_data
        input_post = json.dumps(self.request_input)
        url =  "https://api.brightdata.com/datasets/v3/scrape?dataset_id=gd_lyy3tktm25m4avu764&notify=false&include_errors=true&type=discover_new&discover_by=company_url"
        headers = {
        ...
        if status == 200:
            ...
            if snapshot_id:
                ...
                time.sleep(10)
                return self.sync_linkedin_post_by_url_company(input_data)
        else:
            return status, None
        
    def sync_linkedin_profile_by_url_company(self, input_data:list[dict]):
        self.request_input['input'] = input_data
        input_post = json.dumps(self.request_input)
        url =  "https://api.brightdata.com/datasets/v3/scrape?dataset_id=gd_l1vikfnt1wgvvqz95w&notify=false&include_errors=true"
        headers = {
        "Authorization": f"Bearer {self.api_key}",
        "Content-Type": "application/json",
        }
        ...
        if status == 200:
            ...
            if snapshot_id:
                ...
                print(f"Snapshot is not Ready, Wait 10 seconds")
                time.sleep(10)
                return self.sync_linkedin_profile_by_url_company(input_data)
        else:
            return status, None
    ...
    def save_upload_s3(self, s3_key:str):
        ...
        profile_name = f"linkedin-profile-{channel_name}-{timestamp_data}.parquet"
        post_name = f"linkedin-post-{channel_name}-{timestamp_data}.parquet"
        ...
        try:
            ...
            self.logger.info(f'file linkedin {channel_name} parquet is deleted')
        except Exception as err:
...
# lk_scrapper = LinkedinScrapper(api_key=api_key)

# football_club = [
#     {
#         "url":"https://www.linkedin.com/company/fc-barcelona/",
#     },
#     {
...
# status, result = lk_scrapper.sync_linkedin_profile_by_url_company(football_club)
# lk_scrapper.write_json_file(status, result, "linkedin_profile.json")
...
# status, result = lk_scrapper.sync_linkedin_post_by_url_company(football_club)
# lk_scrapper.write_json_file(status, result, "linkedin_post.json")
